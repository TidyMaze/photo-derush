#!/usr/bin/env python3
"""Improve keep/trash model using learnings from AVA experiments.

Key improvements:
1. Add feature interactions (biggest impact)
2. Add regularization (rsm, subsample)
3. Use better CatBoost params
4. Optional: ResNet50 embeddings
"""
...
log = logging.getLogger("improve_keep_trash")
...
def align_and_concat(X_feats: np.ndarray, filenames: list[str], emb: np.ndarray, emb_fnames: list[str]):
    """Align embeddings with features by filename."""
    import os
...
def prepare_features_with_interactions(X_handcrafted, embeddings_path=None, pca_dim=128, use_resnet50=False):
    """Prepare features with interactions (AVA learning #1)."""
    X = X_handcrafted
    ...
    log.info(f"Base features: {X_combined.shape[1]} (handcrafted + embeddings)")
    
    # Add feature interactions (AVA learning #1 - biggest impact)
...
def train_improved_model(X_train, X_val, X_test, y_train, y_val, y_test, use_ava_params=True):
    """Train model with AVA-learned improvements."""
    from catboost import CatBoostClassifier
    
    if use_ava_params:
        # Best params from AVA experiments
        cb_params = {
            "iterations": 2500,
            "learning_rate": 0.018,
            "depth": 7,
            "l2_leaf_reg": 3.0,
            "rsm": 0.88,  # Feature dropout (AVA learning #3)
            "bootstrap_type": "Bernoulli",  # Required for subsample
            "subsample": 0.85,  # Row dropout (AVA learning #3)
            "random_seed": 42,
            "verbose": 50,
            "thread_count": -1,
            "loss_function": "Logloss",
            "early_stopping_rounds": 200,  # Prevent overfitting
            "eval_metric": "Accuracy",
            "use_best_model": True,
        }
        log.info("Using AVA-learned hyperparameters with regularization")
    else:
        # Load existing best params
        from src.tuning import load_best_params
        xgb_params = load_best_params() or {}
        # Map XGBoost params to CatBoost (simplified)
        cb_params = {
            "iterations": xgb_params.get("n_estimators", 200),
            "learning_rate": xgb_params.get("learning_rate", 0.1),
            "depth": xgb_params.get("max_depth", 6),
            "l2_leaf_reg": xgb_params.get("reg_lambda", 1.0),
            "random_seed": 42,
            "verbose": 50,
            "thread_count": -1,
            "loss_function": "Logloss",
        }
        log.info("Using existing tuned hyperparameters")
...
def main():
    parser = argparse.ArgumentParser(description="Improve keep/trash model with AVA learnings")
    ...
    parser.add_argument("--use-resnet50", action="store_true", help="Use ResNet50 embeddings")
    parser.add_argument("--pca-dim", type=int, default=128, help="PCA dimension")
    parser.add_argument("--use-ava-params", action="store_true", default=True, help="Use AVA-learned params")
    ...
    log.info("="*80)
    log.info("IMPROVING KEEP/TRASH MODEL WITH AVA LEARNINGS")
    log.info("="*80)
    ...
    if args.embeddings:
        ...
        embeddings_path = ".cache/embeddings_resnet50.joblib"
    else:
        # Try to find existing embeddings
    ...
    X_final, transformer = prepare_features_with_interactions(
        X, embeddings_path, args.pca_dim, args.use_resnet50
    )
    ...
    model, metrics = train_improved_model(
        X_train, X_val, X_test, y_train, y_val, y_test, args.use_ava_params
    )
    
    # Compare to baseline (if known)
    baseline_acc = 0.8667  # From BEST_MODEL_DESCRIPTION.md
    ...
    if improvement > 0:
        log.info("\n✅ Model improved with AVA learnings!")
    else:
        log.info("\n⚠️  Model didn't improve (may need more data or different approach)")
    
    # Save model
    if args.model_path:
        model_path = args.model_path
    else:
        model_path = os.path.expanduser("~/.photo-derush-improved-model.joblib")
