...
def analyze_group(group_id: int, image_dir: str):
    ...
    if len(hashes) < 2:
        ...
        print(f"  Pairwise distances (showing first 10):")
        ...
        if distances:
            ...
            if max_dist <= PHASH_HAMMING_THRESHOLD:
                ...
                print(f"     This suggests grouping may be due to burst merging, not hash similarity")
    ...
    if len(burst_ids) == 1:
        ...
        print(f"  ✅ Grouped by burst merging (same burst)")
    elif len(burst_ids) > 1:
        print(f"  Images span {len(burst_ids)} different bursts: {sorted(burst_ids)}")
        print(f"  ⚠️  Grouped despite different bursts - likely hash similarity")
    ...
    for idx, (filename, info) in enumerate(group_images, 1):
        ...
        if make or model:
            ...
            print(f"     Camera: unknown (no EXIF Make/Model)")
    ...
    if len(burst_ids) == 1:
        ...
        if max_dist <= PHASH_HAMMING_THRESHOLD:
            ...
            print(f"Reason: Mixed (hash similarity + burst merging)")
    else:
        print(f"Reason: Unable to determine (insufficient hash data)")
